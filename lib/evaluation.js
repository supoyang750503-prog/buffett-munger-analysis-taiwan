...
async function generateEvaluation(stockId, companyName, providedAnalysisData, providedValuation) {
  ...
  // 供應鏈查詢跟4次RAG檢索彼此互不依賴，全部平行執行，
  // 避免在 Vercel 這類有執行時間上限的環境裡因排隊等待而逾時
  const [supplyChainResult, ...ragResults] = await Promise.all([
    getSupplyChain(stockId, companyName).catch(() => null),
    ...QUALITATIVE_DIMENSIONS.map((dim) =>
      ragSearch(dim.query, geminiKey, index).then((passages) => ({
        key: dim.key,
        label: dim.label,
        passages,
      }))
  ...
  const supplyChain = supplyChainResult;
